...
**수정 리스트 (25/04/01)**
※ 기존 별도 파일 history_TA현황분석.md의 내용을 본 파일로 통합 (해당 파일은 삭제됨).
...
**수정 리스트 (26/06/04)**
※ 상담 유형 전면 개편·키워드 신규 페이지의 기능 상세는 상단 [상담 유형] / [키워드] 섹션 참조. 본 항목은 같은 커밋(67879f6)의 나머지 변경분을 보강 기록.

[리스크 키워드 용어 통일 — "이슈 키워드" → "리스크 키워드" (TA 전체)]
- UI 표기를 TA 전 페이지에서 "리스크 키워드"로 통일 (call-status / issue-manage / consultation-type / report-generator 등)
- 데이터 필드명 `issueKeywords`는 내부 식별자로 유지 — 화면 표기만 변경 (위 문서의 일부 "이슈 키워드" 표기는 통일 이전 시점 기준 설명)

[AI 현황분석 sub-item 드롭다운 (call-status.html)]
- "이슈 상위 N종" · "상담 흐름" 등 다수 값을 헤더(펼침 토글) + chip 목록(최대 3) 구조로 표시
- chip 클릭 시 해당 값 기준으로 드릴다운/필터 (L1→L2, L2→L3, L3는 테이블 필터)

[키워드 페이지 신규 등록 (sidebar.js)]
- 사이드바 TA 메뉴에 키워드(keyword-analysis) 항목 등록 — menuItems에 { id, label, href } 추가

[누락분 소급 기록]
- TA 대시보드 전체화면(풀스크린) 모드 (26/03/30, edafe85) — 대시보드를 iframe 풀스크린으로 표시
- 콘텐츠 영역 최대 너비 제한 max-width:1744px (26/03/31, 6fd619c) — 1920×1080 기준 좌우 여백 정리
...
- `_gen_ta_xlsx.py` : TA 기능정의서 출력 경로를 하드코딩(`d:\work\AICC_QA\01_문서\...`)에서 스크립트 위치 기준 상대경로(`os.path.dirname(__file__)/01_문서/...`)로 교체 — 폴더 이동에도 안전
